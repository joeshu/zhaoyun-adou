...
function dealDmg(S, m, dmg, byUnit, cell) {
  ...
  else if (G && G.mode === 'fire' && S === G.E) { G.fire.oil = Math.min(G.fire.oilMax, G.fire.oil + 1); }   // 火攻：每击杀曹军回充控火油
  ...
  if (S.killCnt >= pk) {
    ...
    // 长坂独胆：敌方侧(G.E)不参与模拟，压力怪在此模式无意义；赤壁火攻：玩家侧不出军，压力怪不可落到 G.P
    if (G.mode !== 'escort' && G.mode !== 'fire' && G.mode !== 'rogue' && G.mode !== 'siege' && opp.mobs.length < cap) {
...
function updMob(S, m, dt) {
  if (G.mode === 'rogue') { rogueUpdMob(S, m, dt); return; }   // 试炼：跨侧真实交火，绕过原生 mob-vs-cells
  if (G.mode === 'siege') { siegeUpdMob(S, m, dt); return; }   // 反向攻城：玩家突击队专用行军+破城，绕过 mob-vs-cells
  const mb = MOBS[m.type];
...
// 反向攻城：玩家突击队专用行军 + 抵达末端砸城（绕过原生 mob-vs-cells；不调 hurtAdou，siege 无敌行军至阿斗）
function siegeUpdMob(S, m, dt) {
  if (m.flash > 0) m.flash -= dt;
  if (m.stun > 0) { m.stun = Math.max(0, m.stun - dt); return; }
  const sg = G.siege;
  const spd = m.spd * (sg.rushT > 0 ? SIEGE_RUSH_SPD : 1);   // 突进令生效期间移速倍增
  m.d += spd * dt;
  const p = pathPos(S.path, S.cum, m.d);
  m.x = p.x; m.y = p.y;
  if (m.d >= S.len) {                 // 抵达末端 → 砸城
    const dmg = m.break * SIEGE_BREAK_MUL * (sg.focusT > 0 ? SIEGE_FOCUS_BREAK : 1);   // 集火令额外倍率
    sg.fort.hp -= dmg;
    m.dead = true;
    boom(m.x, m.y, '#8a6d3b');
    popFloat(sg.fort.x, sg.fort.y + 22, Math.round(dmg), null, { txt: '-' + Math.round(dmg), col: '#8a6d3b' });
    if (sg.fort.hp <= 0) {
      sg.fort.hp = 0;
      endBattle(true);
      G.rewardTxt = '反向攻城·敌垒已破';
    }
  }
}

// 反向攻城：敌塔杀玩家兵——独立结算，不含任何玩家奖励/连杀/压力怪路径（镜像 rogueDealDmg，§7.7）
function siegeDealDmg(m, dmg) {
  if (!m || m.hp <= 0) return;
  m.hp -= dmg; m.flash = 0.12;
  if (m.hp > 0) return;
