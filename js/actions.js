...
function manualUlt(S) {
  ...
  if (ok) {
    best.u.cd = skillCd(best.u); sfx('skill');
    // 手动大招专属全屏演出（#7）：金色闪光 + 中央扩散环 + 横幅，与觉醒（fxRing+boomRadial+shake+banner）区分但同调
    G.ultFx = { t: 0.9, t0: 0.9, hero: best.u.name };
    addShake(5);
...
function placeCards(S, cards) {                             // 入栏；栏满→玩家侧进临时背包(≤3)，再满才折现
  ...
  if (over) { S.mantou += over * 2; if (S.side > 0) popFloat(187, 545, 'mantou', over * 2); }   // 栏满溢出折现 = 馒头入账（#6 经济可见）
  return placed;
}
function doSummon(S) {
...
function dropUnit(S, a1, i1, a2, i2) {
  ...
  if (o.type === 'hero') {
    ...
    if (S.side > 0 && G) popFloat(dst.x, dst.y - 24, 'heal', null, { txt: '共鸣·回血', col: '#2f9e44' });
      if (S.side > 0) recOp({ op: 'drop', a1, i1, a2, i2 });
      return 'hero';
    }
...
function recycleUnit(S, area, idx) {
  ...
  if (S.side > 0) {
    popFloat(RECYCLE.x + RECYCLE.w / 2, RECYCLE.y, 'mantou', v);   // 回收折现 = 馒头入账（#6）
